...
`npm run shipit` validates the game and runs the Node deployment script. The same command works from Codex on macOS and Windows. It uses the operating system's standard `ssh`, `scp`, and `tar` commands; it does not need Fish or `rsync`.
